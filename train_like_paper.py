...
def get_train_pairs(step_i, batch_size, images, labels, config, num_actions, num_regression_output, models, bs, random_init = True):
    ...
    ind = np.random.randint(img_count, size = config.batch_size)
...
def main():
    ...
    #num_shape_params = num_landmarks * 3 <- 나는 num_shape_params를 6으로 설정해야 맞을듯..
    ...
    shape_model.eval()
    print(f"Shape model loaded! (w/ val loss: {hist['val_loss']})")
    models['model'] = model
